...
import {
  ...
  updateTarifasPublicasMasivo,
  getTarifasPublicas,
  getConfiguraciones,
} from '@/features/admin/tarifas/actions';
import { FiEdit3, FiSave, FiX, FiZap, FiPackage, FiDollarSign } from 'react-icons/fi';
...
export function TarifasPublicasTable({ initialTarifas, initialConfig }) {
  const [tarifas, setTarifas] = useState(() =>
    initialTarifas.map((t) => ({ ...t, precio_por_kg_usd: Number(t.precio_por_kg_usd) }))
  );
  ...
  const refresh = async () => {
    ...
    try {
      const [newTarifas, newConfig] = await Promise.all([getTarifasPublicas(), getConfiguraciones()]);
      setTarifas(newTarifas.map((t) => ({ ...t, precio_por_kg_usd: Number(t.precio_por_kg_usd) })));
      setConfig(newConfig);
    } catch { /* silent */ }
  ...
  const enterEditMode = () => {
    const snap = {};
    tarifas.forEach((t) => (snap[t.peso] = t.precio_por_kg_usd));
  ...
  const getCurrentPrice = (tarifa) =>
    editedPrices[tarifa.peso] !== undefined ? editedPrices[tarifa.peso] : tarifa.precio_por_kg_usd;
    ...
    try {
      if (Object.keys(editedPrices).length > 0) {
        const payload = Object.entries(editedPrices).map(([peso, precio]) => ({
          ...
          precio_por_kg_usd: parseFloat(precio),
        }));
        const result = await updateTarifasPublicasMasivo(payload);
        if (!result?.success) {
  ...
  const configDefs = [
    {
      field: 'recargo_express',
      label: 'Recargo Express (%)',
      subtitle: 'Recargo para envíos express',
      suffix: '%',
      icon: FiZap,
      iconBg: 'bg-violet-50',
    ...
    {
      field: 'peso_limite_tarifa_publica',
      label: 'Peso Límite',
    ...
    {
      field: 'precio_kg_extra_publico',
      label: 'Kg Extra (Público)',
  ...
  return (
    <>
          ...
          <div className="p-4 border-b border-slate-200 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:block">
                Tarifario Público - Utilizado cuando no hay cupón de descuento aplicado
              </p>
              {!isEditMode && (
                <button
          ...
          <div className="overflow-y-auto" style={{ maxHeight: '70vh' }}>
            <table className="w-full">
              ...
              <tbody>
                {tarifas.length === 0 ? (
                  <tr>
                    <td colSpan="2" className="px-6 py-12 text-center text-slate-500">No hay tarifas cargadas</td>
                  </tr>
                ) : (
                  tarifas.map((tarifa, index) => (
                    ...
                    >
                      <td className="px-4 py-2 text-center border-r border-slate-200 border-b border-b-slate-100">
                        <span className="text-sm font-medium text-slate-600 font-mono">{tarifa.peso.toFixed(1)}</span>
                      </td>
                      <td className="px-4 py-2 text-center border-b border-b-slate-100">
                        {isEditMode ? (
                          ...
                          <span className="text-sm text-slate-800 font-mono">
                            {Number(tarifa.precio_por_kg_usd).toFixed(2)}
                          </span>
                        )}
                      </td>
                    </tr>
        ...
        <div className="fixed bottom-4 md:bottom-8 left-0 right-0 z-40 px-4">
          <div className="max-w-7xl mx-auto bg-slate-900 rounded-2xl shadow-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 w-full md:w-auto justify-center md:justify-start">
              ...
              <span className="text-sm font-medium text-white">Editando tarifas públicas...</span>
              <span className="pl-3 text-sm text-slate-300">{totalPriceChanges} precio(s) modificado(s)</span>
            </div>
            <div className="flex gap-3 w-full md:w-auto">
              ...
              <button
                ...
                className={`flex-1 md:flex-none justify-center px-5 py-2.5 rounded-xl border font-semibold text-sm transition-all flex items-center gap-2 ${hasValidationErrors || totalPriceChanges === 0 || isBusy
                  ...
                  : 'border-emerald-500 bg-emerald-600 text-white hover:bg-emerald-700 cursor-pointer active:scale-95'
                  }`}
              >
      ...
      <ConfirmModal
        ...
        message={`Estás a punto de guardar **${totalPriceChanges} precio(s)** en tarifas públicas. ¿Confirmar?`}
